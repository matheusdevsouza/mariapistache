...
export async function getProductVariants(productId: number): Promise<any[]> {
  try {
    ...
    console.warn('Erro ao buscar variantes do produto:', error);
    return [];
  }
}
...
export async function getProductColorVariations(productId: number): Promise<any[]> {
  try {
    const sql = `
      SELECT id, product_id, color_name, color_hex, sku, price, original_price, 
             stock_quantity, is_default, is_active, sort_order, created_at, updated_at
      FROM product_color_variations 
      WHERE product_id = ? AND is_active = TRUE
      ORDER BY sort_order ASC, color_name ASC
    `
    console.log('Buscando color variations para product_id:', productId);
    const result = await query(sql, [productId]);
    console.log('Resultado da query color variations:', result);
    console.log('Número de variações encontradas:', result?.length || 0);
    return result || [];
  } catch (error: any) {
    console.error('Erro ao buscar color variations:', error);
    console.error('Mensagem de erro:', error?.message);
    if (error?.message?.includes('does not exist') || error?.message?.includes('não encontrada')) {
      console.warn('Tabela product_color_variations não encontrada, retornando array vazio');
      return [];
    }
    throw error;
  }
...
export async function getProductColorVariants(productId: number): Promise<any[]> {
  return [];
}

export async function getProductStats(productId: number): Promise<any> {
  return {
    views: 0,
    clicks: 0,
    purchases: 0
...
export async function getVariationImages(productId: number, variationId: number): Promise<any[]> {
  return [];
}
...
export async function getUserById(id: number): Promise<any> {
  ...
  if (row) {
    try {
      const decrypted = decryptFromDatabase('users', row);
...
export async function getUserByUuid(uuid: string): Promise<any> {
  const sql = `SELECT * FROM users WHERE user_uuid = ? AND is_active = TRUE`;
...
async function createVerificationToken(userId: number, token: string): Promise<any> {
  ...
  `;
  return await query(sql, [userId, hashedToken]);
}
export async function getVerificationToken(token: string): Promise<any> {
  ...
  const sql = `
    ...
    WHERE vt.token = ? AND vt.expires_at > NOW() AND vt.is_used = FALSE
  `;
...
export async function markVerificationTokenAsUsed(token: string): Promise<any> {
  const hashedToken = hashTokenValue(token);
  const sql = `UPDATE email_verification_tokens SET is_used = TRUE, used_at = NOW() WHERE token = ?`;
  return await query(sql, [hashedToken]);
}
...
export async function getPasswordResetToken(token: string): Promise<any> {
  ...
  const sql = `
    ...
    WHERE prt.token = ? AND prt.expires_at > NOW() AND prt.is_used = FALSE
  `;
  const result = await query(sql, [hashedToken]);
  return result[0] || null;
}
export async function markPasswordResetTokenAsUsed(token: string): Promise<any> {
  const hashedToken = hashTokenValue(token);
  const sql = `UPDATE password_reset_tokens SET is_used = TRUE, used_at = NOW() WHERE token = ?`;
  return await query(sql, [hashedToken]);
}
