  ...
  try {
    ...
    const [
      ...
      database.query(`
        SELECT 
          COUNT(*) as total,
          COUNT(CASE WHEN created_at >= ?::timestamp THEN 1 END) as newThisPeriod
        FROM users
      `, [startDate.toISOString()]),
      database.query(`
        SELECT 
          SUM(CASE WHEN created_at >= ?::timestamp THEN total_amount ELSE 0 END) as current,
          SUM(CASE WHEN created_at >= ?::timestamp AND created_at < ?::timestamp THEN total_amount ELSE 0 END) as previous
        FROM orders
      `, [startDate.toISOString(), previousStartDate.toISOString(), previousEndDate.toISOString()]),
      database.query(`
        SELECT 
          o.id,
          o.order_number,
